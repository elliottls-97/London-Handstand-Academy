...
function cleanSkills(x) {
  ...
  put('l', n(x.l, 2)); put('kick', n(x.kick, 1)); put('ctw', n(x.ctw, 3)); put('tuck', n(x.tuck, 2)); put('free', n(x.free, 3));
  if (Array.isArray(x.adv)) out.adv = x.adv.map(v => String(v).slice(0, 12)).filter(v => /^(press|onearm|shapes|none)$/.test(v)).slice(0, 4);
  if (Array.isArray(x.path)) out.path = x.path.map(v => String(v).slice(0, 8)).filter(v => /^(l|kick|ctw|tuck|free|adv)$/.test(v)).slice(0, 6);
